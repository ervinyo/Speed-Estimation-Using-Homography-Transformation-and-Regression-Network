...
def modified_matrices_calculate_range_output_without_translation(height, width, overhead_hmatrix,
    ...
    if out_upperpixel > out_lowerpixel:
        ...
        max_height = height * 10 #---------FOR UPDATE THE HEIGHT OF THE IMAGE------------
...
def main(_):
    ...
    scaled_overhead_hmatrix, target_dim = get_scaled_homography(overhead_hmatrix, 400 * 2, est_range_u, est_range_v)
    ...
    cv2.imshow('Results', img_rotated)
    cv2.waitKey(0)
