...
def modified_matrices_calculate_range_output_without_translation(height, width, overhead_hmatrix,verbose=False):
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        scaled_overhead_hmatrix, target_dim = get_scaled_homography(overhead_hmatrix, 860 * 2, est_range_u, est_range_v)     
        warped = cv2.warpPerspective(img_cv, scaled_overhead_hmatrix, dsize=target_dim, flags=cv2.INTER_CUBIC)

        median_angle = None
        if isRotate :
            img_before = warped.copy()
        ...
        while video.isOpened() :
            ...
            if isRotate :
                warped = ndimage.rotate(warped, median_angle)
            
            if output_video is None :
                output_video = cv2.VideoWriter(outputPath, cv2.VideoWriter_fourcc(*"mp4v"), video_fps, (warped.shape[1], warped.shape[0]))
            output_video.write(warped)
            
            # cv2.imshow("Result", warped)
            
            Avi.PrintPercent(video.get(cv2.CAP_PROP_POS_FRAMES), video.get(cv2.CAP_PROP_FRAME_COUNT), "Processing : ")
            
            '''
            diff = (time.time() - timer) * (frame_count - counter)
            timer = time.time()
            counter += 1
            
            print("{}/{} - {}               \r".format(counter, frame_count, hhmmss(diff, 1)), end='')
            '''
